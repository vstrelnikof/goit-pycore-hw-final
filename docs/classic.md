...
| `contacts search [пошук] [N]` | 📇 Пошук контактів (опційно термін і/або ліміт N); у пошуку можна використовувати `*` (wildcard) |
...
| `notes search [пошук] [N]` | 📝 Пошук нотаток (опційно термін і/або ліміт N); у пошуку можна використовувати `*` (wildcard) |
...
У пошуковому терміні символ **`*`** означає «будь-які символи» (наприклад, `jo*n` знайде «john», «jon», «jordan» тощо).

Якщо результатів більше за вказаний ліміт, під таблицею з’являється підсумок *«Показано N з M»*.

## 💡 Вгадування команд

Програма завжди намагається вгадати команду та підкоманду, введені користувачем і одразу виконує їх (через `difflib`). Якщо команду не знайдено виводиться повідомлення:

```
❓ Невідома команда. Введіть help — список доступних команд.
```
