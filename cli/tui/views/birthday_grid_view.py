...
class BirthdayGridView(BaseGridView):
    ...
    def __init__(self, screen: Screen, state: AppState):
        super().__init__(screen, state, title="🎂 Дні народження")
    ...
    def _filter_list(self):
        days_to_show: int | None = (
            int(self._search_box.value)
            if self._search_box.value.isnumeric()
            else days_left_in_year()
        )
        table_data = self._state.address_book_manager.get_birthdays_table_data(
            days_to_show
        )
